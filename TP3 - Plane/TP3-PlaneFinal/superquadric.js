...
function superquadricInit(gl, e1, e2, nlat, nlon) {
    superquadric_points = [];
    ...
    superquadricBuild(nlat, nlon, e1, e2);
    superquadricUploadData(gl);
}

// Generate points using polar coordinates
function superquadricBuild(nlat, nlon,e1,e2) 
{
    ...
    for(var i=0, phi=Math.PI/2-d_phi; i<nlat; i++, phi-=d_phi) {
        for(var j=0, theta=0; j<nlon; j++, theta+=d_theta) {
            var pt = vec3(r* Math.sign(Math.cos(phi))*Math.pow(Math.abs(Math.cos(phi)),e1)* Math.sign(Math.cos(theta))*Math.pow(Math.abs(Math.cos(theta)),e2),
            r* Math.sign(Math.sin(phi))*Math.pow(Math.abs(Math.sin(phi)),e1),
            r* Math.sign(Math.cos(phi))*Math.pow(Math.abs(Math.cos(phi)),e1)* Math.sign(Math.sin(theta))*Math.pow(Math.abs(Math.sin(theta)),e2)   );
...
function superquadricDraw(gl, program, filled=false) {
